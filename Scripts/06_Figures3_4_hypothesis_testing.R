...
##### fluxes to CDOM
      ...
      png(file=paste(path_figures,"Figure14_fluxes_cdom_alternative.png",sep=""), width=1600, height=580, pointsize=26, family="Helvetica")      
        #par(mfrow=c(2,2))
...
###### % CDOM in total exudation
        ...
        legend(x=120,y=0.065, legend=c(expression(italic("Constant")),expression(italic("Nutrients")),expression(italic("Optimized"))),
               col=colores[elegidos[c(3,2,1)]], bty="n", lwd=anchol, lty=1)     
        text(x=109,y=0.063, pos=3, label="(0-9m)", cex=cexlabs+0.2, font=1)                
        text(x=146,y=0.063, pos=3, label="DCM",    cex=cexlabs+0.2, font=1)
        
        
##### total fluxes CDOM  (0-9m)    
        plot(x=1, y=1, type="n", col=colores[i], lwd=1, las=1, cex.lab=1.0, pch=19, cex=0.2,
             main="", ylab=expression(paste(X["C"]^"(2)"," (mgC ",m^-3,"",d^-1,")", sep="  ")), xlab="",
        ...
        legend(x="topright", legend=c(expression(italic("Constant")),expression(italic("Nutrients")),expression(italic("Optimized"))),
               col=c(colores[elegidos[c(3,2,1)]]), bty="n", lwd=anchol)         
        legend(x=-20, y=1.0, legend="(0-9m)", cex=1.2,  bty="n")
        
        
    # total fluxes DCM     
        plot(x=1, y=1, type="n", col=colores[i], lwd=1, las=1, cex.lab=1.0, pch=19, cex=0.2,
             main="", ylab=expression(paste(X["C"]^"(2)"," (mgC ",m^-3,"",d^-1,")", sep="  ")), xlab="",
             xlim=c(1,363), xaxs="i", ylim=c(0,0.3), yaxs="i", mgp=c(2.7,1,0), xaxt="n")
        axis(1, at=seq(15,362-14,by=30), labels=c("J","F","M","A","M","J","J","A","S","O","N","D"), las=1, cex.axis=cexaxis)
        ...
        legend(x="topright", legend=c(expression(italic("Constant")),expression(italic("Nutrients")),expression(italic("Optimized"))),
               col=c(colores[elegidos[c(3,2,1)]]), bty="n", lwd=anchol)             
        legend(x=-20, y=0.4, legend="DCM", cex=1.2,  bty="n")
...
###### acdom(450)        
        ...
        legend(x="topright", legend=expression(paste(a[CDOM],"(450) (0-9m) (", m^-1,")")), cex=1.1,bty="n")
        ...
        legend(x="topright", legend=expression(paste(a[CDOM],"(450) (40-60) (", m^-1,")")), cex=1.1, bty="n")
...
#################
  ...
  png(file=paste(path_figures,"Figure15_bleaching_acdom_profiles.png",sep=""), width=1000, height=1000, pointsize=24, family="Helvetica")
        ...
        layout(matrix(c(1,2,      5,3,6,4, 2, 1, rep(2,6),  # B O B O
                        ...
                        1,17, 20,18,19,21,17, 1, rep(17,6)),  ncol=7, byrow=T), widths = c(0.5,0.1,rep(0.2,4),0.4))
        par(mar=c(2,4,1,1))
        par(oma=c(2,1,1,1))
        layout.show(n=21)
        cexdates=1
...
#### degradation
        plot(x=1, y=1, type="n", col=colores[i], lwd=1, las=1, cex.lab=1.0, pch=19, cex=0.2,
             ...
             ylab="", xlab="", yaxt="n")
        mtext(2,at=200,outer=F,line=3,text="m",cex=cexlabel,las=1)
        ...
        mtext(1,at=-3,outer=F,line=2.5,text=expression(paste(degX[C]^3,"+",remX[C]^3," (mgC ", m^-3," ", d^-1, ")",sep="")),cex=0.75)
        ...
        legend(x="bottomright",legend=c(expression(italic("Optimized")),expression(italic("Bleaching"))),
               col=c(colores[elegidos]), bty="o",  lwd=c(3,3))                
        
#### surface  acdom(450)
        plot(x=1, y=1, type="n", col=colores[i], lwd=1, las=1, cex.lab=1.0, pch=19, cex=0.2,
             main="", ylab="", xlab="", xlim=c(1,363), xaxs="i", ylim=c(0,0.07), yaxs="i", mgp=c(2.5,1,0), xaxt="n")
        mtext(2,at=0.035,outer=F,line=3,text=expression(m^-1),cex=cexlabel,las=1)
        axis(1, at=seq(15,362-14,by=30), labels=c("J","F","M","A","M","J","J","A","S","O","N","D"), las=1, cex.axis=cexaxis)
        text(x=c(360,360), y=c(0.060,0.045), pos=2,
               labels=c(expression(paste(a[CDOM],"(450)")),"(0-9m)"),
